...
int siw_mmap(struct ib_ucontext *ctx, struct vm_area_struct *vma)
{
	...
	if (!uobj) {
		siw_dbg(uctx->sdev, "mmap lookup failed: %lu, %u\n",
			off, size);
		goto out;
	}
		  ...
		  const struct ib_send_wr **bad_wr)
{
	...
	while (wr) {
		...
		siw_dbg_qp(qp, "opcode %d, flags 0x%x, id 0x%p\n",
...
int siw_dereg_mr(struct ib_mr *base_mr, struct ib_udata *udata)
{
	...
	siw_dbg_mem((&mr->mem), "deregister MR, #ref's %d\n",
		    kref_read(&mr->mem.ref));
	...
	/*
	 * To avoid accidential re-allocation of same STag index
	 * during a concurrent new MR allocation, MR remains in
	 * idr lookup until destroyed. MR gets destroyed if last
				    ...
				    u64 start, u64 len, int rights)
{
	...
	siw_dbg_mem((&(mr->mem)), "new MR, mem 0x%p\n", &mr->mem);

	mr->base_mr.lkey = mr->base_mr.rkey = mr->mem.stag;

	mr->mem.va = start;
	mr->mem.len = len;
	mr->mem.mr = NULL;
	mr->mem.perms = rights & IWARP_ACCESS_MASK;
	mr->mem_obj = mem_obj;
			      ...
			      u64 rnic_va, int rights, struct ib_udata *udata)
{
	...
	if (udata) {
		...
		mr->mem.stag |= ureq.stag_key;
		uresp.stag = mr->mem.stag;
		  ...
		  unsigned int *sg_off)
{
	...
	if (!pbl) {
		siw_dbg_mem((&mr->mem), "no PBL allocated\n");
		return -EINVAL;
	}
	pble = pbl->pbe;

	if (pbl->max_buf < num_sle) {
		siw_dbg_mem((&mr->mem), "too many SGE's: %d > %d\n",
			    mr->pbl->max_buf, num_sle);
		return -ENOMEM;
	}
	for_each_sg(sl, slp, num_sle, i) {
		if (sg_dma_len(slp) == 0) {
			siw_dbg_mem((&mr->mem), "empty SGE\n");
			return -EINVAL;
		}
		if (i == 0) {
	...
	if (rv > 0) {
		...
		siw_dbg_mem((&mr->mem),
			...
			mr->mem.len, mr->mem.va, num_sle, pbl->num_buf);
	}
	return rv;
}
		   ...
		   struct ib_srq_init_attr *init_attrs, struct ib_udata *udata)
{
	...
	struct siw_pd *pd = to_siw_pd(base_srq->pd);
	struct siw_device *sdev = pd->sdev;
	struct siw_ucontext *ctx =
		rdma_udata_to_drv_context(udata, struct siw_ucontext,
					  base_ucontext);
	...
	siw_dbg_pd(pd, "[SRQ 0x%p]: success\n", srq);
...
{
	struct siw_srq *srq = to_siw_srq(base_srq);
	struct siw_device *sdev = srq->pd->sdev;
	struct siw_ucontext *ctx =
		rdma_udata_to_drv_context(udata, struct siw_ucontext,
					  base_ucontext);
...
out:
	if (unlikely(rv < 0)) {
		siw_dbg_pd(srq->pd, "[SRQ 0x%p]: error %d\n", srq, rv);
		*bad_wr = wr;
	}
	return rv;
}
...
void siw_srq_event(struct siw_srq *srq, enum ib_event_type etype)
{
	...
	if (base_srq->event_handler) {
		siw_dbg_pd(srq->pd, "reporting SRQ event %d\n", etype);
