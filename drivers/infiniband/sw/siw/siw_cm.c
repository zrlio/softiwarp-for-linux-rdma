...
static void siw_cm_work_handler(struct work_struct *w)
{
	...
	siw_dbg_cep(cep, "[QP %u]: work type: %d, state %d\n",
		    cep->qp ? cep->qp->id : -1, work->type, cep->state);
...
int siw_cm_queue_work(struct siw_cep *cep, enum siw_work_type type)
{
	...
	siw_dbg_cep(cep, "[QP %u]: work type: %d, work 0x%p, timeout %lu\n",
		    cep->qp ? cep->qp->id : -1, type, work, delay);
...
int siw_connect(struct iw_cm_id *id, struct iw_cm_conn_param *params)
{
	 ...
	 */
	if (to_sockaddr_in(id->remote_addr).sin_port != 0)
	...
	siw_dbg_qp(qp, "id 0x%p, pd_len %d, laddr %pI4:%d, raddr %pI4:%d\n",
		   id, pd_len,
		   &to_sockaddr_in(laddr).sin_addr,
		   ntohs(((struct sockaddr_in *)(laddr))->sin_port),
		   &to_sockaddr_in(raddr).sin_addr,
...
int siw_accept(struct iw_cm_id *id, struct iw_cm_conn_param *params)
{
	...
	if (params->ord > sdev->attrs.max_ord ||
		...
		siw_dbg_cep(
			...
			id, qp->id, params->ord, sdev->attrs.max_ord,
			params->ird, sdev->attrs.max_ird);
		rv = -EINVAL;
		up_write(&qp->state_lock);
		goto error;
	...
	if (params->private_data_len > max_priv_data) {
		siw_dbg_cep(
			...
			id, qp->id, params->private_data_len, max_priv_data);
		rv = -EINVAL;
		up_write(&qp->state_lock);
		goto error;
	}
	...
	siw_dbg_cep(cep, "id 0x%p, [QP %u]: send mpa reply, %d byte pdata\n",
		    id, qp->id, params->private_data_len);
...
int siw_create_listen(struct iw_cm_id *id, int backlog)
{
	...
	if (to_sockaddr_in(id->local_addr).sin_family == AF_INET) {
		...
		siw_dbg(sdev,
			...
			id, &to_sockaddr_in(id->local_addr).sin_addr,
		...
		for_ifa(in_dev)
		{
			 ...
			 */
			if (ipv4_is_zeronet(to_sockaddr_in(id->local_addr)
						    .sin_addr.s_addr) ||
			    to_sockaddr_in(id->local_addr).sin_addr.s_addr ==
				    ifa->ifa_address) {
				laddr.sin_addr.s_addr = ifa->ifa_address;

				siw_dbg(sdev, "id 0x%p: bind: %pI4:%d\n",
					id, &laddr.sin_addr,
					ntohs(laddr.sin_port));

				rv = siw_listen_address(
					id, backlog, (struct sockaddr *)&laddr);
				if (rv)
					break;
			}
		}
